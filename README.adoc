...
. You must have Java JDK 11 installed.  Java 8 is nt supported by Hibernate 6.x.
...
* Run `mvn -Dgpg.skip install` - see [project README](https://github.com/nuodb/HibernateDialect6/blob/master/README.md)
...
Modify properties in [`databases/nuodb/resources/hibernate.properties`](databases/nuodb/resources/hibernate.properties). 

* DO NOT change the database name or credentials as they are used by Bamboo.
...
The JAR version is required in several places and should pick up the version from the environment variable (therefore no other changes are necessary).
