...
public class XmlSources {
	...
	public static XmlSource fromResource(String resourceName, ClassLoaderService classLoaderService) {
		...
		if (url == null) {
			...
			int ix = resourceName.indexOf(".hbm.xml");
			String path = "src/test/java/" + (ix != -1 //
					? resourceName.substring(0, ix).replace('.', '/') + ".hbm.xml" //
			...
			if (f.exists())
				try {
					...
					Logger log = LoggerFactory.getLogger(XmlSources.class);
					log.error(e.getClass().getName() + " failed creating url for " + path + ": "
