...
/**
 * <b>NUODB OVERRIDE CLASS:</b> Add NuoDB specific timeout code - just like all
 * the databases. As this class is defined in the {@code hibernate-testing} JAR
 * we can only modify it by making a local copy.
 * <p/>
 * 
...
public class TransactionUtil {
	...
	public static void setJdbcTimeout(Session session, long millis) {
		...
		session.doWork( connection -> {
			...
			// NuoDB: Start
			else if( dialect instanceof NuoDBDialect ) {
				try (Statement st = connection.createStatement()) {
					//Prepared Statements fail for SET commands
					st.execute(String.format( "SET LOCK_WAIT_TIMEOUT %d", millis/1000 ));
				}
			}
			// NuoDB: End
			else {
