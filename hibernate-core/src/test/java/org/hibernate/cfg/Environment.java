...
public final class Environment implements AvailableSettings {
	...
	static {
        ...
        // Matrix tests specify target/matrix/<db-name> as the current directory and
        // automatically setup the correct hibernate.properties file for the database
        // being tested.
		if (projectDir.getName().equals("hibernate-core")) {
		    File altPropsFile = new File("src/test.resources/hibernate-nuodb.properties");
		    
		    if (!altPropsFile.exists())
                try {
                    Files.copy(new File("../databases/nuodb/resources/hibernate.properties").toPath(), 
                            new File("src/test.resources/hibernate-nuodb.properties").toPath(), StandardCopyOption.COPY_ATTRIBUTES);
                } catch (IOException e) {
		...
		int choice = JOptionPane.showConfirmDialog(null, projectDir);
