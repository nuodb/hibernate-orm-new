...
public class ReuseCriteriaWithMixedParametersTest extends BaseEntityManagerFunctionalTestCase {
    ...
    protected Class<?>[] getAnnotatedClasses() {
        return new Class[] { Wallet.class, Person.class };
    }
    ...
    public void likeCqReuse() {
        doInJPA(this::entityManagerFactory, entityManager -> {
            ...
            // NuoDB 5-Jun-2023: This fails until 'escape ?' is implemented.
            try {
                Query query = entityManager.createQuery(criteriaQuery);
                query.setParameter(stringValueParameter, "Z%");
                ...
                rethrowExceptionUnlessExpected(e);
            }
    ...
    private void rethrowExceptionUnlessExpected(PersistenceException e) {
        if (getDialect() instanceof NuoDBDialect) {
            Throwable cause = ExceptionUtil.findCause(e, SQLSyntaxErrorException.class);
    
            if (cause != null && cause.getLocalizedMessage().contains("expected string literal got ?"))
                return; // Expected, 'escape ?' not yet implemented
        }
